...
val baseVersion = "2.6.7"
version = "${baseVersion}.${getVersionSuffix()}"
...
fun getVersionSuffix(): String {
    ...
    if (output.endsWith("-dirty")) {
        output = output.substring(0, output.length - "-dirty".length)
        suffix = "-dirty"
    }
    if (output.endsWith("-broken")) {
        output = output.substring(0, output.length - "-broken".length)
        suffix = "-broken"
    }
    if ('-' !in output) {
        return "0$suffix"
    ...
    return split[split.size - 2] + suffix
}
